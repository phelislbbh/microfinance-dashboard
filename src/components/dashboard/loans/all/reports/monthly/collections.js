			...
			(period, index) => {
				...
				_.forEach(_.remove(_.concat([], loans), ({ status }) => _.includes([13, 14], status)), entry => {
					...
					if (
						...
						periodSeries.Interest = periodSeries.Principal + parseFloat(interest_remaining);
				...
				_.forEach(loans, entry => {
					...
					if (
						...
						periodSeries.Collected =
							periodSeries.Collected + parseFloat(principal_paid) + parseFloat(interest_paid);
						periodSeries.Remaining =
							periodSeries.Remaining + parseFloat(principal_remaining) + parseFloat(interest_remaining);
