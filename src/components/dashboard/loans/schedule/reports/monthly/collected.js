...
class Collected extends Component {
	renderCount = () => {
		...
		const data = _.map(
			_.map(Configs.enumerateMonths(startDate.toDate(), endDate.toDate()), date =>
			...
			period => {
				...
				_.forEach(schedule, entry => {
					if (entry[`${this.props.filter.type}_date`].startOf('month').format('YYYY-MM-DD') === period) {
		...
		return (
			<Line
				...
				xAxis={{ key: 'period', label: 'Period', formatter: date => moment(date).format('MMM, YY') }}
			/>
		);
	};
	...
	renderActual = () => {
		...
		const data = _.map(
			_.map(Configs.enumerateMonths(startDate, endDate), date => moment(date).format('YYYY-MM-DD')),
			(period, index) => {
				let periodSeries = {
					Amount: 0
				};
			...
			(period, index) => {
				...
				_.forEach(schedule, entry => {
					if (entry[`${this.props.filter.type}_date`].startOf('month').format('YYYY-MM-DD') === period) {
						...
						let delay = moment(date_paid).diff(lastDateAcceptableForPayment, 'months');
						delay = parseInt(delay) <= 1 ? 'On-time' : `${delay} months late`;
